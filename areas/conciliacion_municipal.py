...
def analizar_conciliacion_municipal(df):
    ...
    try:
        ...
        # =================================================================
        kpis_row = dbc.Row([
            dbc.Col(_kpi_card("ti-handshake", "Arreglos conciliatorios", f"{int(total_arreglos):,} acuerdos", "Resolución pacífica de conflictos", VERDE), width=12, sm=6, lg=3, className="mb-3"),
            dbc.Col(_kpi_card("ti-coin", "Pensión alimenticia", f"{int(total_pensiones):,} gestiones", "Protección familiar y jurídica", GUINDA), width=12, sm=6, lg=3, className="mb-3"),
            dbc.Col(_kpi_card("ti-shield", "Canalizaciones a seguridad pública", f"{int(total_seguridad):,} casos", "Coordinación operativa", VERDE), width=12, sm=6, lg=3, className="mb-3"),
        ...
        for i, (_, r) in enumerate(df_resumen.iterrows()):
            ...
            filas_tabla.append(html.Tr([
                html.Td(str(r[col_actividad]), style={**td_style, "backgroundColor": bg, "fontWeight": "600"}),
                html.Td(str(r[col_variable]), style={**td_style, "backgroundColor": bg}),
        ...
        fila_total = html.Tr([
            html.Td("TOTAL GENERAL DE ATENCIONES Y GESTIONES DE CONCILIACIÓN", style={**td_style, "fontWeight": "700", "color": "#fff", "backgroundColor": VERDE_DARK}),
        ...
        tabla_layout = html.Div([
            ...
            html.Table([
                html.Thead(html.Tr([
                    html.Th("Actividad Registrada", style={**th_sticky, "width": "50%"}),
                    html.Th("Variable / Clasificación", style={**th_sticky, "width": "35%"}),
        ...
        if not df_tendencia.empty and df_tendencia[col_cantidad_sistema].sum() > 0:
            ...
            seccion_grafica = _chart_panel("Dinámica mensual de atenciones y gestiones", fig_tendencia, color_top=GUINDA)
        else:
