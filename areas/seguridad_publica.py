...
# Colorimetría unificada basada en la segunda imagen (Predominio Verde Oscuro e institucional)
VERDE_PRINCIPAL = "#0f4c3a"  # Tono principal de la segunda imagen
GUINDA_INST = "#691c32"      # Acento secundario (Guinda)
DORADO_INST = "#bc955c"      # Acento complementario (Dorado)
TEXTO_DARK = "#1f2937"
...
def analizar_seguridad_publica(df):
    ...
    try:
        ...
        col_comunidad = columnas_reales[0] if len(columnas_reales) > 0 else "Comunidad"
        col_reportes = next((c for c in columnas_reales if "REPORTES" in c.upper()), columnas_reales[1] if len(columnas_reales) > 1 else "Reportes")
        col_faltas = next((c for c in columnas_reales if "FALTAS" in c.upper()), columnas_reales[2] if len(columnas_reales) > 2 else "Faltas")
        col_accidentes = next((c for c in columnas_reales if "ACCIDENTES" in c.upper()), columnas_reales[3] if len(columnas_reales) > 3 else "Accidentes")
        col_catastrofes = next((c for c in columnas_reales if "CATASTROFES" in c.upper()), columnas_reales[4] if len(columnas_reales) > 4 else "Catastrofes")
        col_mp = next((c for c in columnas_reales if "MINISTERIO PÚBLICO" in c.upper() or "MINISTERIO PUBLICO" in c.upper()), columnas_reales[5] if len(columnas_reales) > 5 else "MP")
        col_fgr = next((c for c in columnas_reales if "FISCALIA" in c.upper() or "FGR" in c.upper()), columnas_reales[6] if len(columnas_reales) > 6 else "FGR")
        col_ubicacion = next((c for c in columnas_reales if "UBICACIÓN" in c.upper() or "UBICACION" in c.upper()), columnas_reales[7] if len(columnas_reales) > 7 else "Ubicación")

        for col in [col_reportes, col_faltas, col_accidentes, col_catastrofes, col_mp, col_fgr]:
        ...
        latitudes, longitudes, links_mapa = [], [], []
        ...
        # Tarjetas KPI con la barra lateral en verde principal (estilo segunda imagen)
        tarjetas_kpi = dbc.Row([
            dbc.Col(
                html.Div([
                    html.Div(style={"position": "absolute", "top": "0", "left": "0", "bottom": "0", "width": "5px", "backgroundColor": VERDE_PRINCIPAL, "borderRadius": "8px 0 0 8px"}),
                    html.Small("COMUNIDADES ATENDIDAS 🖱️", className="font-weight-bold d-block", style={"fontSize": "0.68rem", "letterSpacing": "0.5px", "color": TEXTO_SECUNDARIO}),
                    html.H3(f"{total_comunidades_atendidas} Localidades", className="m-0 font-weight-bold mt-1", style={"color": TEXTO_DARK, "fontSize": "1.25rem"}),
                    html.Small("Ver listado general", className="d-block font-weight-bold", style={"fontSize": "0.58rem", "marginTop": "3px", "color": VERDE_PRINCIPAL})
                ], id="kpi-seguridad-click", className="p-3 position-relative h-100", style=estilo_kpi), width=12, sm=6, md=3, className="mb-3"
            ),
            dbc.Col(
                html.Div([
                    html.Div(style={"position": "absolute", "top": "0", "left": "0", "bottom": "0", "width": "5px", "backgroundColor": GUINDA_INST, "borderRadius": "8px 0 0 8px"}),
                    html.Small("REPORTES CIUDADANOS", className="font-weight-bold d-block", style={"fontSize": "0.68rem", "letterSpacing": "0.5px", "color": TEXTO_SECUNDARIO}),
                    html.H3(f"{total_reportes:,.0f}", className="m-0 font-weight-bold mt-1", style={"color": VERDE_PRINCIPAL, "fontSize": "1.25rem"})
                ], className="p-3 position-relative h-100", style=estilo_kpi), width=12, sm=6, md=3, className="mb-3"
            ),
            dbc.Col(
                html.Div([
                    html.Div(style={"position": "absolute", "top": "0", "left": "0", "bottom": "0", "width": "5px", "backgroundColor": VERDE_PRINCIPAL, "borderRadius": "8px 0 0 8px"}),
                    html.Small("ACCIDENTES DE TRÁNSITO", className="font-weight-bold d-block", style={"fontSize": "0.68rem", "letterSpacing": "0.5px", "color": TEXTO_SECUNDARIO}),
                    html.H3(f"{total_accidentes:,.0f}", className="m-0 font-weight-bold mt-1", style={"color": TEXTO_DARK, "fontSize": "1.25rem"})
                ], className="p-3 position-relative h-100", style=estilo_kpi), width=12, sm=6, md=3, className="mb-3"
            ),
            dbc.Col(
                html.Div([
                    html.Div(style={"position": "absolute", "top": "0", "left": "0", "bottom": "0", "width": "5px", "backgroundColor": GUINDA_INST, "borderRadius": "8px 0 0 8px"}),
                    html.Small("PUESTAS A DISPOSICIÓN (MP / FGR)", className="font-weight-bold d-block", style={"fontSize": "0.68rem", "letterSpacing": "0.5px", "color": TEXTO_SECUNDARIO}),
                    html.H3(f"{total_puestas:,.0f}", className="m-0 font-weight-bold mt-1", style={"color": VERDE_PRINCIPAL, "fontSize": "1.25rem"})
                ], className="p-3 position-relative h-100", style=estilo_kpi), width=12, sm=6, md=3, className="mb-3"
            ),
        ], className="mb-2")
        ...
        desplegable_tabla = dbc.Collapse(
            html.Div([
                html.Div([
                    html.I(className="bi bi-table me-2"), "REGISTROS INTERNOS: INCIDENCIA Y ATENCIÓN POR COMUNIDAD"
                ], style={"backgroundColor": VERDE_PRINCIPAL, "color": "white", "padding": "8px 12px", "fontWeight": "bold", "fontSize": "0.75rem", "borderRadius": "6px 6px 0 0"}),
                html.Div([
                    dash_table.DataTable(
                        ...
                        style_data_conditional=[
                            ...
                            {'if': {'column_id': 'UBICACION_MAPA_BTN'}, 'color': VERDE_PRINCIPAL, 'fontWeight': 'bold', 'cursor': 'pointer'}
                        ]
                    )
        ...
        return html.Div([
            dcc.Store(id='store-mapa-seguridad-seleccion', data={
                "lat": 21.155597, 
                "lng": -98.903757, 
                "zoom": 11.2, 
                "comunidad": "",
            ...
            dbc.Row([
                dbc.Col(html.Div([
                    html.Div([
                        ...
                        "VISOR GEOGRÁFICO INTERACTIVO DE SEGURIDAD PÚBLICA Y ATENCIÓN MUNICIPAL"
                    ], style={'backgroundColor': VERDE_PRINCIPAL, 'color': 'white', 'padding': '12px 16px', 'fontWeight': '700', 'fontSize': '0.75rem', 'borderRadius': '6px 6px 0 0'}),
                    html.Iframe(
        ...
        return dbc.Alert(f"⚠️ Error al construir estadísticas: {str(e)}", color="danger", className="m-3")
...
<body>
    ...
    <script>
        ...
        function getColor(totalAcciones) {{
            ...
            if (t > 20) return "#0f4c3a"; // Verde principal (Alta incidencia)
            if (t > 10) return "#691c32"; // Guinda (Media-Alta)
            if (t > 5) return "#bc955c";  // Dorado (Media)
            if (t > 0) return "#2563eb";  // Azul (Baja)
            return "#9ca3af";             // Gris (Sin registros)
        }}
