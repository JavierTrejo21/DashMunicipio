...
# ------------------------------------------------------------------------------
# 1. GENERACIÓN DE RESUMEN MIR DESDE EXCEL (COLUMNAS ESPECÍFICAS)
# ------------------------------------------------------------------------------
def generar_resumen_indicadores_area(nombre_area):
    """Lee el Excel DES01_CHU_02_2026.xlsx y extrae las columnas requeridas

    (G, I, J, M, O, P, Y, AC, AG, AK, AM, AN, AO) filtrando únicamente por el
    área seleccionada.
    """
    ...
    try:
        # Cargar ignorando encabezados iniciales para detectar la fila de títulos
        ...
        # Carga completa con el encabezado detectado
        df_excel = pd.read_excel(ruta_excel, header=fila_encabezado)

        # Mapeo de índices de columnas por letra (G=6, I=8, J=9, M=12, O=14, P=15, Y=24, AC=28, AG=32, AK=36, AM=38, AN=39, AO=40)
        indices_cols = [6, 8, 9, 12, 14, 15, 24, 28, 32, 36, 38, 39, 40]
        indices_validos = [
        ...
        # Filtrar por Columna C (índice 2 - Unidad Responsable)
        col_ur_idx = 2
        ...
        # Si no hay coincidencia exacta, se intenta por la clave del área (ej. 5.1.1)
        ...
        # Extraer únicamente las columnas seleccionadas
...
def diseñar_tarjeta_pbr(datos_pbr):
    ...
    if isinstance(datos_pbr, dict):
        ...
        color_alerta = color_map.get(
            datos_pbr.get("estatus_semaforo"), "light"
        )
        return dbc.Alert(
            [
                html.H5(
                ...
                dbc.Row([
                    dbc.Col(
                        [
                            html.P(
                                f"📊 Cumplimiento:"
                                f" {datos_pbr.get('porcentaje_cumplimiento', 0)}%",
                                className="mb-1 font-weight-bold",
                                style={"fontSize": "0.8rem"},
                            ),
                            html.P(
                                "📋 Metas Programadas:"
                                f" {datos_pbr.get('total_metas_programadas', 0)}",
                    ...
                    dbc.Col(
                        [
                            html.P(
                                f"✅ Metas Alcanzadas:"
                                f" {datos_pbr.get('total_metas_alcanzadas', 0)}",
                                className="mb-1 font-weight-bold",
                                style={"fontSize": "0.8rem"},
                            ),
                            html.P(
...
def construir_tabla_estilo_cards(df, pagina_actual=1):
    ...
    cols_a_mostrar = [
        c for c in df.columns if c.lower() not in ["rowid", "id"]
    ]
    columnas_tarjetas = []
    ...
    barra_paginacion = html.Div(
        [
            html.Div(
                [
                    ...
                    html.Span(
                        f"{inicio + 1} - {min(fin, total_registros)}",
                        className="fw-bold text-dark",
                        style={"fontSize": "0.78rem"},
                    ),
                    html.Span(
                        f" de {total_registros}",
                        className="text-muted",
                        style={"fontSize": "0.78rem"},
                    ),
                ],
        ...
        className=(
            "d-flex justify-content-between align-items-center mt-3 px-2 py-2"
...
def diseñar_tabla_mir_consolidada(df_mir):
    if df_mir.empty:
        return html.Div(
            "No hay registros disponibles en la Matriz MIR.",
            className="text-center text-muted p-4",
        )

    ESTILO_NIVEL = {
        "FIN": {"bg": "#0D9488", "color": "#FFFFFF"},
        "PROPÓSITO": {"bg": "#691C32", "color": "#FFFFFF"},
        "COMPONENTE": {"bg": "#334155", "color": "#FFFFFF"},
        "ACTIVIDAD": {"bg": "#E2E8F0", "color": "#1E293B"},
    }

    cols_a_mostrar = [
        c for c in df_mir.columns if str(c).lower() not in ["rowid", "id"]
    ]

    # --- ESTRUCTURA DE ENCABEZADOS DE DOS NIVELES ---
    ...
    header_nivel_1 = html.Tr([
        html.Th(
            ...
            style={
                ...
                "fontWeight": "800",
                "textAlign": "center",
                "borderRight": "2px solid #042F2E",
                "letterSpacing": "0.8px",
            },
        ),
        html.Th(
            "DATOS DEL INDICADOR Y METAS",
            colSpan=max(1, len(cols_a_mostrar) - 5),
            style={
                "backgroundColor": "#064E3B",
                "color": "#FFFFFF",
                "padding": "8px",
    ...
    # Fila 2: Nombres exactos de las columnas
    headers_nivel_2 = [
        html.Th(
            str(col).replace("_", " ").upper(),
            style={
    ...
    for idx_row, row in df_mir.iterrows():
        ...
        for col in cols_a_mostrar:
            ...
            if "NIVEL" in str(col).upper():
                nivel_key = str(val).strip().upper()
                estilo_badge = ESTILO_NIVEL.get(
                    nivel_key, {"bg": "#CBD5E1", "color": "#0F172A"}
                )
        ...
        filas_tabla.append(html.Tr(celdas, style={"backgroundColor": bg_fila}))
    ...
    boton_ver_general = dbc.Button(
        [
            html.I(
                ...
                style={"fontSize": "1rem", "color": "#0D9488"},
            ),
            html.Span(
        ...
        color="light",
        className=(
            "w-100 mb-3 shadow-sm border d-flex align-items-center"
    ...
    encabezado_mir = html.Div(
        [
            html.I(
                className="bi bi-clipboard-data-fill me-2",
                style={"color": "#691C32", "fontSize": "1rem"},
            ),
            html.Span(
                "MATRIZ DE INDICADORES PARA RESULTADOS (MIR CONSOLIDADA)",
                style={
                    "fontWeight": "800",
                    "color": "#691C32",
                    "fontSize": "0.82rem",
    ...
    tabla_html = html.Table(
        [
            html.Thead([header_nivel_1, header_nivel_2]),
    ...
    return html.Div([boton_ver_general, encabezado_mir, contenedor_mir])


# ------------------------------------------------------------------------------
# 4. REGISTRO DE CALLBACKS (EXPUESTO A NIVEL PRINCIPAL)
# ------------------------------------------------------------------------------
def register_navegacion_callbacks(app):
    ...
    def cargar_acuerdos(_):
        ...
        if df.empty:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "CREATE TABLE IF NOT EXISTS acuerdos (id INTEGER PRIMARY KEY,"
                " nombre TEXT)"
            )
            cursor.executemany(
            ...
            tarjetas.append(
                dbc.Col(
                    html.Div(
                        ...
                        className=(
                            "p-4 bg-white border text-center position-relative"
                            " h-100 shadow-sm"
                        ),
                        style={
                            "borderRadius": "18px",
    ...
    def mostrar_dashboard(n_clicks):
        ...
        info_est = DICCIONARIO_AREAS.get(
            ...
            {
                "resumen": (
                    "Área operativa integrada en los acuerdos del Plan"
                    " Municipal."
                ),
                "objetivo": (
                    "Seguimiento y evaluación continua de los indicadores"
                    " sectoriales."
                ),
            },
        )
        ...
        if tabla not in tablas_existentes:
            ...
            aviso = dbc.Alert(
                [
                    ...
                    html.P(
                        f"El área '{nombre_area}' está registrada pero aún no"
                        " contiene datos importados en la tabla SQLite"
                        f" '{tabla}'."
                    ),
                    html.Hr(),
                    html.Small(
                        "Utiliza el botón de administración en el encabezado"
                        " para cargar el Excel correspondiente."
                    ),
                ],
                color="warning",
            )
        ...
        try:
            ...
            bloque_resumen = dbc.Alert(
                [
                    ...
                    html.Hr(style={"margin": "6px 0"}),
                    html.Small(
                        f"🎯 OBJETIVO GENERAL: {info_est['objetivo']}",
                        className="text-muted font-italic",
                        style={"fontSize": "0.75rem"},
                    ),
                ],
                color="light",
                className="mt-2 mb-3 border-start border-primary",
                style={"borderLeftWidth": "4px"},
            )

            store_pagina = dcc.Store(id="store-pagina-actual", data=1)

            # Extraer directamente la MIR desde el Excel DES01_CHU_02_2026.xlsx
            resumen_mir_excel = generar_resumen_indicadores_area(nombre_area)

            # Contenedor para las tarjetas de la tabla operativa SQLite
            ...
            contenido = html.Div([
                ...
                bloque_resumen,
                resumen_mir_excel,  # <-- Inyección directa de la tabla con doble encabezado
                vista_tabla,
    ...
    def renderizar_tabla_paginada(pag_actual, _, active_info):
        ...
        try:
            ...
            return construir_tabla_estilo_cards(
                df, pagina_actual=pag_actual or 1
            )
        except Exception:
            conn.close()
            return no_update
